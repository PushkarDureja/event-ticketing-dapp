...
import { handleError, isAddress, toEvent } from "../helpers/utils";
...
class WithdrawalForm extends Component {
  ...
  withdraw = async (event) => {
    ...
    try {
      await contract.methods.withdrawMoney(event.id).send({
        from: accounts[0],
      });
  ...
  renderCheck = () => {
    const { accounts, contract } = this.props;
    const { addressToCheck, event, isAddressValid, isValidatingAddress } =
      this.state;
        ...
        onSubmit={async (e) => {
          ...
          if (isValidAddress) {
            const result = await contract.methods
              .participantHasTicket(addressToCheck, event.id)
  ...
  renderWithdraw = () => {
    const { event, isWithdrawalSucceed, isWithdrawing } = this.state;
    ...
    if (isWithdrawing) {
      ...
      if (canWithdraw) {
        ...
        message = <p>You can withdraw money after the event ended.</p>;
      }
    }
    ...
    return (
      <div className="mt-2">
        {message}
        {canWithdraw && hasMoneyToWithdraw && !isWithdrawalSucceed && (
          <Button
            variant="primary"
            block
  ...
  render() {
    ...
    const { activeTab, event, isWithdrawalSucceed, isWithdrawing, loaded } =
      this.state;
    ...
    return (
      ...
      >
        <Modal.Header closeButton={!isWithdrawing || isWithdrawalSucceed}>
          <Modal.Title>{loaded ? event.name : "Loading..."}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {loaded ? (
            <Tabs
